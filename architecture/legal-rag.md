...
일상용어 → 법령용어 매핑 데이터는 사용자 질문에서 검색 확장어를 만들기 위해 사용한다.
...
사용자 질문의 일상어를 법령 검색에 적합한 확장어로 사용한다.
...
`use_for_rewrite`, `use_for_expansion`은 사용하지 않는다.  
문장 변환 없이 모든 매핑 결과는 검색 확장어 후보로만 사용한다.

---

## 8. 사용자 질문 입력 슬롯

사용자 질문은 아래 형태의 입력 슬롯으로 처리한다.

```json
{
  "raw_question": "집을 살 때 알아야 할 법이 있을까?",
  "matched_terms": [
    {
      "source_term": "집",
      "legal_terms": ["주택", "부동산"]
  ...
  "embedding_text": "사용자 질문: 집을 살 때 알아야 할 법이 있을까?\n검색 확장어: 주택, 부동산, 취득, 매수, 매매, 매매계약, 실거래신고, 자금조달계획서, 취득세, 소유권이전등기"
}
```

| 슬롯 | 역할 |
|---|---|
| `raw_question` | 사용자가 입력한 원문 |
| `matched_terms` | 사용자 질문에서 발견된 일상어와 대응 법령용어 목록 |
| `expanded_terms` | 검색에 추가할 법령용어 목록 |
| `embedding_text` | 최종 임베딩 대상 문장 |
