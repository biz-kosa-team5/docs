...
- 실행 명령: `server/.venv/bin/python scripts/run_chatbot_qa.py --from-docs --allow-failures`
- OpenAI API key가 없는 환경이라 supervisor 초기화는 credential error를 기록했지만, deterministic fallback 경로 기준 전체 144건은 통과했다.
- 위치 조회 fallback answer는 좌표 숫자 대신 `지도에 표시했습니다` 문구를 사용한다.
- JSONL payload에는 `uiActions`, `uiArtifacts`, `uiSummary`가 포함된다.
- 대표 확인:
  - 단지 위치/실거래 결과는 `focus_map` complex action을 생성하고 첫 action만 `autoRun=true`다.
  - 추천 결과는 후보 action과 `recommendation_list` artifact를 생성한다.
  - 시세 추이 결과는 `trend_line_chart` artifact를 생성한다.
  - 법령/미지원 질문은 지도 action과 artifact를 생성하지 않는다.
...
| MX-LLM-002 | chatbot.qa.aggregate.supervisor | PASS | supervisor_aggregate | supervisor_initialization_failed | recommendation, price_trend | - | Y | Y | docs expectation: supervisor_llm / supervisor_aggregate 또는 known_gap |
...
- answer: 래미안대치팰리스 위치는 대치동 1027입니다. 지도에 표시했습니다.
...
- answer: 잠실엘스 위치는 잠실동 19입니다. 지도에 표시했습니다.
...
- answer: 잠실엘스 위치는 잠실동 19입니다. 지도에 표시했습니다. 질문 임베딩을 생성할 수 없어 법령 검색을 실행하지 못했습니다.

### MX-FR-002

- question: 잠실엘스 위치 알려줘 그리고 오늘 날씨 알려줘
- answer: 잠실엘스 위치는 잠실동 19입니다. 지도에 표시했습니다. 오늘 날씨 알려줘는 처리하지 못했습니다. 지원 가능한 질문은 단지 조회, 아파트 추천, 단지 비교, 시세 추이, 계약 법령 질문입니다.
...
- answer: 1. 조회된 데이터 기준으로는 다음 후보를 우선 검토할 수 있습니다. 1. (828-21): 최근 거래가 9. 1억원, 가까운 역 강남역(442m), 가까운 교육시설 서울역삼초등학교(493m), 15세대, 사용승인…
...
- answer: 챗봇 실행 준비 중 오류가 발생했습니다. 잠시 후 다시 시도해 주세요.
